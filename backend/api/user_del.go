...
// DeleteUsersRequest represents the JSON request structure for deleting one or more users.
...
func DeleteUsersFromNode(ctx context.Context, node config.NodeConfig, usernames []string, inboundTag string, cfg *config.Config) error {
	...
	if err != nil {
		...
		return fmt.Errorf("failed to delete users from node %s: %w", node.NodeName, err)
	}
...
// DeleteUsersHandler handles HTTP requests to delete one or more users from specified nodes.
func DeleteUserHandler(manager *manager.DatabaseManager, cfg *config.Config) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		cfg.Logger.Debug("Received DeleteUsers HTTP request", "method", r.Method)
		...
		for _, node := range targetNodes {
			...
			go func(node config.NodeConfig) {
				defer wg.Done()
				err := DeleteUsersFromNode(ctx, node, req.Users, req.InboundTag, cfg)
				resultsCh <- result{
		...
		if len(errs) > 0 {
			cfg.Logger.Error("Errors occurred while deleting users", "users", req.Users, "error_count", len(errs), "errors", errs)
		...
		if err := users.SyncUsersWithNode(ctx, manager, nodeClients, cfg); err != nil {
			cfg.Logger.Error("Failed to sync users after deletion", "error", err)
			http.Error(w, fmt.Sprintf(`{"error": "failed to sync users: %v"}`, err), http.StatusInternalServerError)
			return
		}

		cfg.Logger.Info("Users deleted and synced successfully from all nodes", "users", req.Users, "node_count", len(targetNodes))
		w.Header().Set("Content-Type", "application/json; charset=utf-8")

...
func AddUsersToNode(ctx context.Context, node config.NodeConfig, usernames []string, inboundTag string, cfg *config.Config) (*proto.ListUsersResponse, error) {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for i, node := range targetNodes {
			...
			go func(node config.NodeConfig, nodeClient *db.NodeClient) {
				defer wg.Done()
				users, err := AddUsersToNode(ctx, node, req.Users, req.InboundTag, cfg)
				resultsCh <- result{
